...
END
  ...
  awk -v c=$col -v p=$pass '{if ($3 == p) print $1,$2,$c}' tmp.txt | gmt psxy -R -J -Ss1.8 -W0.2p,0 -Cg.cpt -O -K >> $out 
...
echo | gmt psxy -R0.5/3.5/0.5/3.5 -JX4 -B1a/1ansew -K -Y13 -P > $out
...
for passenger in "50" "100" "200"; do
  ...
  for waiting in "0" "0.5" "1"; do
    ...
    for behaviour in "good" "mixed" "bad"; do
      ...
      echo $x $y $n_passengers $b_i >> tmp.txt
    done
  done
done

  # -------------------------------------------
  minmax=`awk '{print $4}' tmp.txt | xargs -n1 | gmt gmtinfo -C` 
  minC=`echo $minmax | awk '{print $1}'`
  maxC=`echo $minmax | awk '{print $2}'`

  #gmt makecpt -Cno_green.cpt -T$minC/$maxC > g.cpt
  gmt makecpt -Cmy_color.cpt -T$minC/$maxC/5 > g.cpt

  # -------------------------------------------
  # Plot the data:
  dX=0;  dY=0; text="50 passengers";   pass=50; col=4; do_map
  dX=0; dY=-5; text="100 passengers";  pass=100; col=4; do_map
  dX=0; dY=-5; text="200 passengers";  pass=200; col=4; do_map
  ...
  # Write the Legend:
  echo 0.6 3.2 DISTRIBUTION OF THE BOARDING PASSENGERS | gmt pstext -R0/10/0/10 -JX10 -F+jBL -W0.4p,0 -G255 -Y-4.2 -X-0.5 -O -K >> $out
